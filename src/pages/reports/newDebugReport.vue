<template>
    <div>
        ...
        <slot v-for="detail in summary.details">
            <div>
                <span style="font-weight: bold; font-size: medium">{{detail.name}}</span>
                <el-popover
                    placement="top-start"
                    width="400"
                    trigger="hover"
                >
                    <pre class="code-block">{{detail.in_out}}</pre>
            ...
            >
                ...
                >
                    <template slot-scope="scope">
                        <span>{{ scope.row.meta_datas.data[0].request.url }}</span>
                    </template>
                </el-table-column>
                ...
                >
                    <template slot-scope="scope">
                        <span
                            :class="scope.row.meta_datas.data[0].request.method">{{ scope.row.meta_datas.data[0].request.method }}</span>
                    </template>
                </el-table-column>-->
                ...
                >
                    <template slot-scope="scope">
                        <span>{{ scope.row.meta_datas.stat.response_time_ms }}</span>
                    </template>
                </el-table-column>
                    ...
                    <template slot-scope="props">
                        <el-tabs>
                            <el-tab-pane label="Request">
                                <pre class="code-block" v-html="handleRequest(props.row.meta_datas.data[0].request)"></pre>
                            </el-tab-pane>
                            <el-tab-pane label="Content">
                                <pre class="code-block" v-text="handleContent(props.row.meta_datas.data[0].response.content)"></pre>
                            </el-tab-pane>
                            <el-tab-pane label="Response">
                                <pre class="code-block" v-text="handleResponse(props.row.meta_datas.data[0].response)"></pre>
                            </el-tab-pane>
                            <el-tab-pane label="Validators" v-if="props.row.meta_datas.validators.length !== 0">
                                <pre class="code-block" v-html="props.row.meta_datas.validators"></pre>
                            </el-tab-pane>
