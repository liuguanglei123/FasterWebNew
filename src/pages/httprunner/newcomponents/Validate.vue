<template>
    ...
    >
        ...
        <el-table-column
            ...
            width="320">
            <template slot-scope="scope">
                <el-input clearable v-model="scope.row.actual" placeholder="实际返回值" :disabled="scope.row.disabled"></el-input>
            </template>
        </el-table-column>
        ...
        <el-table-column
            ...
            width="320">
            <template slot-scope="scope">
                <el-input clearable v-model="scope.row.expect" placeholder="期望返回值" :disabled="scope.row.disabled"></el-input>
    ...
    export default {
        ...
        computed: {
            allvalidate:function(){
                var tmp = [];
                for(var i=0;i<this.srcvalidate.length;i++){
                    this.srcvalidate[i]['disabled'] = true
                    tmp.push(this.srcvalidate[i])
                }
                for(var i=0;i<this.validate.length;i++){
                    this.validate[i]['disabled'] = false
                    tmp.push(this.validate[i])
                }
                if(this.validate.length === 0){
                    tmp.push({expect: '', actual: '', comparator: 'equals', type: 1, disabled: false })
                }
                return tmp;
            }
        },
        ...
        watch: {
            ...
            allvalidate: function () {
                if (this.allvalidate.length !== 0) {
                    this.tableData = this.allvalidate;
                }
            }
        },
